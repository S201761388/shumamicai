        ...
        for( iter = 0; iter < criteria.maxCount && max_center_shift > criteria.epsilon; iter++ )
        {
            ...
            if( iter == 0 && (a > 0 || !(flags & KMEANS_USE_INITIAL_LABELS)) )
            {
                ...
                else
                {
                    //算法中用的随机中心点
                    for( k = 0; k < K; k++ ){
                        int tmp_center = rand()%N;
                        while(whe_center[tmp_center]){
                            tmp_center = rand()%N;
                        }
                    ...
                    if( counters[k] != 0 )//如果有点，则center_dist肯定不可能为0
                    {
                        ...
                        for(int i = 0;i < counters[k];i ++){
                            ...
                            for(int j = 0;j < counters[k];j ++){
                                ...
                                double tmp_dist = 0;
                                if(color_distance[center_contain[k][i]][center_contain[k][j]] > 0){
                                    dist +=
                                    color_distance[center_contain[k][i]][center_contain[k][j]]*
                                    color_num[center_contain[k][j]];
                                }
                                else{
                                    tmp_dist = new_distance(sample1, sample2, dims);
                                    color_distance[center_contain[k][i]][center_contain[k][j]]= tmp_dist;
                                    color_distance[center_contain[k][j]][center_contain[k][i]]= tmp_dist;
                            ...
                            double tmp_dist = 0;
                            if(color_distance[center_contain[k][i]][old_center_index[k]] > 0){
                                dist +=
                                color_distance[center_contain[k][i]][old_center_index[k]]*
                                        color_num[old_center_index[k]];
                            }
                            else{
                                tmp_dist =  new_distance(sample1, sample2, dims);
                                color_distance[center_contain[k][i]][old_center_index[k]]= tmp_dist;
                                color_distance[old_center_index[k]][center_contain[k][i]]= tmp_dist;
                                dist += tmp_dist*
                                        color_num[old_center_index[k]];
                            }
                        ...
                        *  center_contain某个center包含的点             *
                        ...
                        if(labels[tmp_center] < k){
                            ...
                            //这地方中心应该用新中心
                            double tmp_dist;
                            if(color_distance[tmp_center][center_index[labels[tmp_center]]] > 0){
                                compactness -=
                                color_distance[tmp_center][center_index[labels[tmp_center]]]*
                                ...
                                tmp_dist = new_distance(data.ptr<float>(tmp_center),
                                                        ...
                                                        dims);
                                color_distance[tmp_center][center_index[labels[tmp_center]]] = tmp_dist;
                                color_distance[center_index[labels[tmp_center]]][tmp_center] = tmp_dist;
...
//            double sum = 0;
//            for(int i = 0;i < N;i++){
//                if(!whe_center[i]&&iter > 0){//表明不是中心也不是第一次聚类
//                    sample = data.ptr<float>(i);
//                    const float* center = centers.ptr<float>(labels[i]);
//                    double tmp_dist;
//                    if(color_distance[i][center_index[labels[i]]] > 0){
//                        sum += color_distance[i][center_index[labels[i]]]*
//                        color_num[i];//用新的颜色相似度算法替换
//                }
//                else{
//                    tmp_dist = new_distance(sample, center, dims);
//                    color_distance[i][center_index[labels[i]]] = tmp_dist;
//                    color_distance[center_index[labels[i]]][i] = tmp_dist;
            ...
            for( i = 0; i < N; i++ )//遍历所有的点，与哪个中心点距离最小就归到哪个中心中去
            {
                ...
                * counters[k]为第k个聚类中点的个数*/
                ...
                for( k = 0; k < K; k++ )
                {
                    ...
                    t++;
                    double dist;
                    if(color_distance[i][center_index[k]] > 0){
                        dist = color_distance[i][center_index[k]]*
                        color_num[i];//用新的颜色相似度算法替换
                    }else{
                        dist = new_distance(sample, center, dims);
                        color_distance[i][center_index[k]] = dist;
                        color_distance[center_index[k]][i] = dist;
                        dist *= color_num[i];
                    }
                    if( min_dist > dist )
                    {
